( function () {    // Fonction qui s'auto-appelle pour éviter collision et sécurité
    ...
    function openDoors() {
        ...
        // Ajout de la classe contenant la dissimulation chronométrée du menu
        ...
        setTimeout( () => {
            // mofifie le overflow:hidden de l'animation pour permettre le scroll après le délais de déroulement de l'animation d'intro
            supprOverFlHid.style.overflow = "auto";
            // une fois le menu caché : ajoute la class qui modifie le menu non étiré par l'animation de masquage puis retire la class de l'animation qui étire le menu 
            navContainer.classList.add( "isHidden" );
            navContainer.classList.remove( "hideMenu" );
        }, 4700 )
        setTimeout( () => {
            // fait disparaitre le logo centrale dans une animation
            bumperBurger.classList.add( "bumpBurger" );
            
        }, 4500 )
        // défini une valeur "played" dans le session storage pour que l'animation ne se rejoue que si le visiteur ferme la page, sinon, l'animation est considérée comme jouée et ne se relance pas
    ...
    // fonction de débugage : en fonction de la valeur du statut,
    function introDebug() {
